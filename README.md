...
一直以为自己已经看懂看透了crnn，但是后来仔细思考后其实发现自己对整个网络的输出其实不是很明了，处于一种有点懵逼的状态，因此我改了一下demo，把各个部分都输出来了，借此来分析一下整个模型的整体用法。

我输出了几个节点，每次rnn的结果，crnn的结果，进行max后的结果以及transpose后的结果。

第一遍rnn输出了size为[26,1,256]的tensor，因为对于这个算法来说，最终是有26个字符位的，所以会有26个，然后最后一维就是第一次经过blstm后的值了。然后第二次rnn输出的是size为[26,1,37]的，然后这里的output其实就是整个crnn的output，37代表的是每个字符的概率，但是这个值都是负数而且都是负的几百左右。然后通过`_, preds = preds.max(2)`得到每个最后一维的最大值，得到一个[26,1]size的tensor。

所以就这里的blistm而言，输出的y并不是单纯的概率，而是一个设定好大小的tensor。除此之外我想看看输出的256的tensor能不能转成16 * 16的图片然后可视化一下。是这样的，我可视化了一下，发现都是些奇奇怪怪的像素点，完全没有看出来有什么实际意义..从这些像素点里看输出是不可能的。所以其实应该这么来看，双向LSTM虽然内部结构是挺复杂的，但是本质上和全连接层差不多，都是要指定输入大小和输出大小，然后得到的这些output，就和卷积网络一样，到底是什么意思就要看你怎么去用怎么去训练了。
